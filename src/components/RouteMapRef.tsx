...
// Define the Leaflet type to fix TypeScript errors
declare global {
  interface Window {
    L: any;
  }
...
const RouteMap = forwardRef<RouteMapRef, RouteMapProps>(({
  ...
  // Create a function to add and animate a driver marker
  const addDriverMarker = (position: [number, number]) => {
    if (!mapRef.current || !window.L) return;
    
    const L = window.L;
    
    // Remove existing driver marker if exists
    if (driverMarkerRef.current) {
      mapRef.current.removeLayer(driverMarkerRef.current);
    }
    
    // Create driver marker
    driverMarkerRef.current = L.marker(position, {
      icon: L.divIcon({
        className: 'custom-div-icon',
        html: `<div style="background-color:#E53E3E;width:14px;height:14px;border-radius:50%;border:3px solid white;box-shadow:0 0 10px rgba(0,0,0,0.5);"></div>`,
        iconSize: [20, 20],
        iconAnchor: [10, 10]
      })
    }).addTo(mapRef.current);
